...
# # Define safe SELECT pattern - Updated to handle JOINs and complex queries
# SAFE_SELECT_PATTERN = re.compile(
#     r"^\s*SELECT\s+(?!\s*\*\s*(?:,|FROM))([\w\s,\.\(\)]+)\s+FROM\s+(Doctor|Appointments|Patient|Prescriptions)(?:\s+\w+)?(?:\s+(?:INNER\s+)?JOIN\s+(?:Doctor|Appointments|Patient|Prescriptions)(?:\s+\w+)?\s+ON\s+[\w\s=\.]+)*(?:\s+WHERE\s+[\w\s='\.]+)?(?:\s+ORDER\s+BY\s+[\w\s,\.]+)?\s*;?\s*$",
#     re.IGNORECASE
# )

# # Fixed: Keyword intents as proper dictionary
# keyword_intents = {
#     "greeting": ["hello", "hi", "hey", "good morning", "good afternoon", "good evening"],
#     "registration_help": ["register", "sign up", "create account", "new user", "registration"],
#     "login_issue": ["login", "log in", "sign in", "password", "forgot password", "can't login"],
#     "general_navigation": ["navigate", "how to", "where is", "find", "menu", "go to"],
#     "general_help": ["help", "support", "assist", "question", "need help"],
#     "show_doctors": ["doctor", "doctors", "find doctor", "show doctors", "available doctors"],
#     "view_appointments": ["appointment", "appointments", "my appointments", "schedule", "booking"],
#     "restricted_access": ["admin", "database", "system", "server", "config"],
#     "unknown": []
# }
...
#     # Allow SELECT * only if it's not the entire select (has WHERE clause)
#     if re.search(r"SELECT\s+\*\s+FROM\s+\w+\s*(?:;|\s*$)", sql_upper):
#         return True
    
#     # Check if it matches our safe pattern
#     if not sql_query_clean.upper().startswith('SELECT'):
#         return True
        
#     # Additional safety: ensure it only accesses allowed tables
#     allowed_tables = ['DOCTOR', 'APPOINTMENTS', 'PATIENT', 'PRESCRIPTIONS']
...
#     for row in rows:
#         result += f"📅 {row['AppointmentDate']}: Dr. {row['DoctorFirstName']} {row['DoctorLastName']} ({row['Specialization']})\n"
#     return result

# def format_doctor_data(rows):
#     if not rows:
#         return "No doctors found."
#     result = "Here are some available doctors:\n"
#     for row in rows:
#         result += f"👨‍⚕️ Dr. {row['FirstName']} {row['LastName']} — {row['Specialization']}\n"
#     return result

# def detect_intent_with_ai(user_input):
#     try:
#         prompt = f"""
# You are a medical chatbot. Your job is to detect intent.
# Do not generate SQL queries. Return the most likely intent from:
...
#     if intent not in ["greeting", "registration_help", "login_issue", "general_navigation", "general_help"]:
#         if not is_authorized(patient_id):
#             return {
#                 "message": "🚫 *Unauthorized access!* Please log in to access this feature.",
...
#         return {
#             "message": "🚫 *Access denied!* You cannot access doctor or admin data.",
...
#             return {"error": "Blocked for security.", "response": "Sorry, can't show doctor details right now."}
#         rows = execute_sql_query(query)
#         return {"intent": intent, "response": format_doctor_data(rows)}

#     if intent == "view_appointments":
#         # Ensure patient_id is not None or 'None'
...
#         query = f"""
#         SELECT a.AppointmentID, a.AppointmentDate, d.FirstName AS DoctorFirstName,
#         d.LastName AS DoctorLastName, d.Specialization
#         FROM Appointments a
...
#                 "response": "Sorry, can't show appointments now.",
#                 "intent": intent,
#                 "debug_query": query  # For debugging - remove in production
#             }
...
#             return {
#                 "error": rows["error"], 
#                 "response": "Unable to retrieve appointments at this time.",
#                 "intent": intent
#             }
            
#         return {"intent": intent, "response": format_appointment_data(rows)}

#     # Static responses for other intents
#     static_responses = {
#         "greeting": {
#             "message": "👋 Hello! Welcome to PresCrypt. I'm here to help you with your medical needs. How can I assist you today?",
#             "action": "greeting"
#         },
#         "registration_help": {
#             "message": "📝 To register for PresCrypt:\n1. Click on 'Sign Up' button\n2. Fill in your personal details\n3. Verify your email\n4. Complete your medical profile",
...
#         },
#         "general_help": {
#             "message": "ℹ️ I'm here to help! You can ask me about:\n• Finding doctors\n• Viewing appointments\n• General navigation\n• Account issues\nWhat would you like to know?",
#             "action": "general_assistance"
#         },
#         "unknown": {
#             "message": "❓ I'm not sure I understand. Could you please rephrase your question or ask about:\n• Doctors\n• Appointments\n• Account help\n• Navigation",
...
# # Additional utility function for query execution if needed
# def execute_query(query, patient_id=None):
#     """Execute a query with additional safety checks"""
#     if is_malicious_query(query):
#         return {"error": "Query blocked for security reasons", "success": False}
    
#     # Add patient_id filter for sensitive queries if needed
#     if patient_id and "Patient" in query:
#         if f"PatientID = '{patient_id}'" not in query:
#             return {"error": "Unauthorized query", "success": False}
